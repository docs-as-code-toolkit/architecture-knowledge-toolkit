...
* How does an author record that a relation to a retired artifact is
  ...
  noise? Deferred until the first retirements make the warning volume
  observable.
* How does a generated matrix mark an inactive relation, for example a marker
  column, a footnote, or struck-through text? Answered by the implementation:
  an `(inactive)` marker on the relation type cell, carrying the
  `relation-inactive` role. See xref:artifact-lifecycle-status[].
